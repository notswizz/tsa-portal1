...
export default function ShowAvailability({ session, staffDocRef }) {
  ...
  if (loading) {
    return (
      <div className="bg-white shadow-md rounded-2xl overflow-hidden border border-gray-200 p-6">
      ...
      <div>
        ...
        {shows.length === 0 ? (
          ...
          <div className="space-y-6">
            <div>
              <label htmlFor="show-select" className="block text-sm font-semibold text-gray-700">Select a Show</label>
              <div className="mt-2 relative rounded-md shadow-sm">
                <select
                  id="show-select"
                  className="block w-full rounded-md border border-gray-200 bg-white py-2 px-3 text-gray-900 focus:border-pink-400 focus:ring-1 focus:ring-pink-200"
                  value={selectedShow?.id || ''}
                  onChange={e => handleShowSelect(e.target.value)}
                >
                  {shows.map(show => (
                    <option key={show.id} value={show.id}>
                      {show.name} ({format(parseISO(show.startDate), 'MMM d, yyyy')} - {format(parseISO(show.endDate), 'MMM d, yyyy')})
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div>
              <h4 className="text-sm font-semibold text-gray-700 mb-2">Your submitted availability:</h4>
              <div className="flex flex-wrap gap-2">
                {availableDates.length === 0 ? (
                  <span className="text-gray-400 text-sm">No dates selected</span>
                ) : (
                  availableDates.map(date => (
                    <span key={date} className="px-3 py-1 rounded-full border border-pink-400 text-pink-600 bg-white text-xs font-medium">
