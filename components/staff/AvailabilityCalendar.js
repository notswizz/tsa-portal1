      ...
      <div className="bg-black bg-opacity-40 rounded-lg p-3">
        <div className="flex items-center justify-between mb-4">
          <button
            ...
            className="p-2 rounded-full bg-pink-500 hover:bg-pink-600 text-white transition-all shadow-md"
          >
        ...
        <div className="grid grid-cols-7 gap-1 rounded-lg overflow-hidden border border-pink-500 shadow-lg">
          {/* Calendar day headers */}
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
            <div key={day} className="bg-pink-500 py-2 text-center text-sm font-semibold text-white">
              {day}
            </div>
          ))}
          ...
          {Array.from({ length: calendarDays[0]?.getDay() || 0 }).map((_, index) => (
            <div key={`empty-start-${index}`} className="bg-gray-900 h-18 p-1"></div>
          ))}
          ...
          {calendarDays.map((day) => {
            ...
            return (
              <div 
                key={day.toISOString()} 
                className={`relative bg-gray-900 border-b border-r border-pink-500 border-opacity-20 h-18 p-1 hover:bg-black transition-all ${
                  isToday ? 'ring-2 ring-pink-500' : ''
                }`}
              >
                <div className={`font-medium text-sm ${isToday ? 'text-pink-500 font-bold' : 'text-pink-200'}`}>
                  {format(day, 'd')}
                </div>
          ...
          {Array.from({ length: 6 - (calendarDays[calendarDays.length - 1]?.getDay() || 0) }).map((_, index) => (
            <div key={`empty-end-${index}`} className="bg-gray-900 h-18 p-1"></div>
          ))}
        </div>
        ...
        <div className="mt-3 flex items-center justify-center space-x-6 text-xs">
          <div className="flex items-center">
            <div className="w-4 h-4 rounded bg-pink-500 mr-2 shadow-sm"></div>
            <span className="text-pink-300">Available</span>
          </div>
          <div className="flex items-center">
            <div className="w-4 h-4 rounded bg-gray-800 mr-2 shadow-sm"></div>
